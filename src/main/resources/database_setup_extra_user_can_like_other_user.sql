...
DROP TABLE album;
DROP TABLE user;
DROP TABLE user_friend_of_user;
...
CREATE TABLE user_friend_of_user(
    id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER REFERENCES user(id),
    user_friend_id INTEGER REFERENCES user(id)
);
...
SELECT u.name AS 'User', u2.name AS 'User liked' FROM user_friend_of_user AS uf
JOIN user AS u ON uf.user_id = u.id
JOIN user AS u2 ON uf.user_friend_id = u2.id;